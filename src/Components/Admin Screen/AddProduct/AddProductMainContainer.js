...
class AddProductMainContainer extends Component {
  ...
  handleProductSubmit = (value) => {
    ...
    if (allproductImages.length !== 3) this.handleProductImageErrors();
    ...
    if (allproductImages.length === 3 && logoImage !== dummyImage) {
  ...
  storeProductImages = (images) => {
    ...
    if (imgLen >= 3) {
      ...
      if (imgLen + storedLen <= 3) {
        l = imgLen;
        if (imgLen + storedLen === 3) this.handleProductImageErrorsRemove();
        else if (imgLen + storedLen < 3) this.handleProductImageErrors();
      } else l = imgLen - (imgLen + storedLen - 3) - 3;
    }
    for (let i = 0; i < l; i++) {
      const reader = new FileReader();
